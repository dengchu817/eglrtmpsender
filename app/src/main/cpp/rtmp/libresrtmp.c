 ...
 */
 JNIEXPORT jlong JNICALL Java_com_example_dengchu_eglrtmpsender_Rtmp_RtmpClient_open
 (JNIEnv * env, jobject thiz, jstring url_, jboolean isPublishMode) {
	 const char *url = (*env)->GetStringUTFChars(env, url_, 0);
	 LOGE("RTMP_OPENING:%s",url);
	...
	if (rtmp == NULL) {
 	...
 	if (!ret) {
 		RTMP_Free(rtmp);
 		rtmp=NULL;
		LOGE("RTMP_Connect=ret");
 		return NULL;
 	}
	 LOGE("RTMP_Connect=ret = %d\n", ret);
 ...
 * Signature: ([CI)I
 */
 JNIEXPORT jint JNICALL Java_com_example_dengchu_eglrtmpsender_Rtmp_RtmpClient_read
 ...
 * Signature: ([CI)I
 */
 JNIEXPORT jint JNICALL Java_com_example_dengchu_eglrtmpsender_Rtmp_RtmpClient_write
 (JNIEnv * env, jobject thiz,jlong rtmp, jbyteArray data, jint size, jint type, jint ts) {
    ...
    packet->m_nInfoField2  =  ((RTMP*)rtmp)->m_stream_id;
	 ...
	 LOGE("write data type: %d, ts %d", type, ts);
    ...
    if (!ret) {
		LOGE("end write error %d", sockerr);
		return sockerr;
    }else
    {
		LOGE("end write success");
		return 0;
    }
}
 ...
 * Signature: ()I
 */
 JNIEXPORT jint JNICALL Java_com_example_dengchu_eglrtmpsender_Rtmp_RtmpClient_close
...
JNIEXPORT jstring JNICALL Java_com_example_dengchu_eglrtmpsender_Rtmp_RtmpClient_getIpAddr
